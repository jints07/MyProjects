...
σz = 0.02; # Standard deviation of technology shock
...
Const = U(param) - derivative_1'*param[1:nparam] + 0.5*param[1:nparam]'*derivative_2*param[1:nparam];
...
# GET MATRIX P USING VAUGHAN'S METHOD
H1 = [A_tilde zeros(nstate+1,nstate+1);
      -Q_tilde eye(nstate+1)];
H2 = [eye(nstate+1) B_tilde*inv(R)*B_tilde';zeros(nstate+1,nstate+1) A_tilde'];
H = inv(H1)*H2;
Λ,V = eig(H);
idx = sortperm(-Λ);
Λ = Λ[idx];
V = V[:,idx];
V11 = V[1:nstate+1,1:nstate+1];
V21 = V[nstate+2:size(V,1),1:nstate+1];
P = V21*inv(V11);
F = inv(R + β*B'*P*B)*(β*B'*P*A + W');
